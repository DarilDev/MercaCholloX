...
### Añadido — 2026-08-22 (continuación 2)
- **Fase 7: Aldi integrado**, cuarta cadena. Aldi no tiene tienda online (confirmado en directo: `buscar.html`/`carrito.html` → 404, la FAQ de la web lo dice explícitamente) — la única fuente de precio real es el folleto de ofertas semanales, pero resultó estar servido como JSON estructurado dentro del `__NEXT_DATA__` de Next.js, no como imágenes escaneadas: no hizo falta ningún paso de OCR/IA. `aldi_client.py` parsea `GET /ofertas.html` → `apiData` → `OFFER_GET.res.algoliaDataMap`, filtrando los ~80 productos de bazar/textil/jardín (`mainCategoryID=None`) para quedarse con los ~90 de alimentación reales. Limitación real y aceptada: a diferencia de Mercadona/Dia/HiperDino, esto es un folleto, no un catálogo permanente — solo cubre lo que esté en oferta esa semana, sin búsqueda por término. Verificado en local: 90 productos reales con marca/precio/pasillo/unidad. Workflow manual (`refresh_aldi_prices.yml`), sin `schedule` — el folleto solo cambia semanalmente.
